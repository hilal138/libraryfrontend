...
import { BookOpen, Send, Book, PlusCircle, LogOut, Menu, X } from "lucide-react";
...
export default function LibrarianLayout({ children }) {
  ...
  const menu = [
    { name: "Book Requests", path: "/librarian/requests", icon: <Send size={20} /> },
    { name: "Return Requests", path: "/librarian/returns", icon: <Send size={20} /> },
    { name: "Issued Books", path: "/librarian/issued", icon: <Book size={20} /> },
    { name: "All Books", path: "/librarian/books", icon: <BookOpen size={20} /> },
    { name: "Add Book", path: "/librarian/add-book", icon: <PlusCircle size={20} /> }
  ];
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 bg-gradient-to-br from-emerald-600 to-emerald-800 z-50 p-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-lg font-bold text-white">AMU Librarian</span>
        </div>
        <button
      ...
      <div className={`fixed inset-y-0 left-0 z-40 w-64 bg-gradient-to-b from-emerald-600 to-emerald-800 transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${
        ...
        <div className="p-6 border-b border-emerald-500/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Librarian</h2>
              <p className="text-xs text-emerald-200">Library Staff</p>
            </div>
          </div>
        </div>
